...
### Final 方法的作用:
1. **方法锁定**:防止任何继承类修改它的含义。处于设计的考虑，要确保在继承中使方法行为保持不变，并且不会被覆盖。
2. **效率**：在SE5、SE6的时候，这样可以消除方法调用的开销（同意编译器调用的方法都转为内嵌调用），但在后续版本，其实只需要把这个事情交给编译器（Compiler和JVM）就好。

现在多数使用final的主要目的都是为了方法锁定


### Final方法 和 Private方法的区别
1. Private的作用就是不让其他人调用、修改。所以其实所有Private方法都隐式的，无法覆盖
